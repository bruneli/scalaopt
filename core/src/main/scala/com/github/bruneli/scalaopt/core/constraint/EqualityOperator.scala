 ...
 * Equality operator
 *
 * @param eps precision used to match the equality constraint
 * @author bruneli
 */
case class EqualityOperator(eps: Double = 1.0e-8) extends ConstraintOperator {
   ...
   */
  def apply(left: Double, right: Double): Boolean = Math.abs(left - right) < eps
