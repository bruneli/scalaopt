...
/**
 * Unconstrained continuous optimization methods that do not require the computation of derivatives
 *
 * @tparam A configuration parameters type
 * @author bruneli
 */
trait DerivativeFreeMethod[A <: ConfigPars] extends Optimizer[UnconstrainedVariable, ContinuousObjectiveFunction[UnconstrainedVariable], A]
