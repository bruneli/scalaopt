...
import com.github.bruneli.scalaopt.core.variable.{Constants, ContinuousVariable, ConversionsFromDouble, PositiveVariables}
...
//
//    val lp = PrimalTableau.min((x: ContinuousVariablesType) => x(0) + x(1))
//
//    lp.subjectTo(((x: ContinuousVariablesType) => 0.5 * x(0) + 1.0 * x(1)) >= 1.0)
//
//    val xOpt = lp.solveWith(StandardSimplex)
//
...
//    val xMin = PositiveVariables(0.0, 1.0)
//
//    val xOpt = DualTableau.min((x: ContinuousVariablesType) => x(0) + x(1))
...
//
//    val xOpt = PrimalTableau.min((x: ContinuousVariablesType) => x(0) + x(1))
//        .solveWith(StandardSimplex)
//
//    xOpt shouldBe 'success
//    for ((xObs, xExp) <- xOpt.get.zip(xMin)) {
...
//    val xOpt = PrimalTableau.min((x: ContinuousVariablesType) => x(0) + x(1))
//        .subjectTo(
//          ((x: ContinuousVariablesType) => 0.5 * x(0) + 1.0 * x(1)) >= -1.0,
//          ((x: ContinuousVariablesType) => x(0).x) >= -1.0
//        )
...
//
//    val xOpt = PrimalTableau.min((x: ContinuousVariablesType) => x(0) + 2.0 * x(1))
//        .subjectTo(
//          ((x: ContinuousVariablesType) => 1.0 * x(0)) >= -2.0,
...
//
//    val xOpt = PrimalTableau.max((x: ContinuousVariablesType) => x(0) + x(1))
//        .subjectTo(
...
//    val xMax = Vector(1.0, 2.0)
//
//    val xOpt = DualTableau.max((x: ContinuousVariablesType) => x(0) + x(1))
//        .subjectTo(
//          ((x: ContinuousVariablesType) => 1.0 * x(0)) <= 1.0,
...
//      PrimalTableau.max((x: ContinuousVariablesType) => x(0) + x(1))
//          .subjectTo(
//            ((x: ContinuousVariablesType) => x(0).x) <= 1.0,
//            ((x: ContinuousVariablesType) => x(1).x) <= 1.0,
//            ((x: ContinuousVariablesType) => x(0) + x(1)) >= 3.0
//          )
//          .solveWith(StandardSimplex)
//    }
//
...
//        .subjectTo(
//          ((x: ContinuousVariablesType) => 1.0 * x(0)) <= 1.0,
//          ((x: ContinuousVariablesType) => 1.0 * x(1)) <= 1.0
//        )
//        .withNegativeVariables
//        .solveWith(StandardSimplex)
...
//    val balancingCoefficients = ones[ContinuousVariable](nOffers)(fromDouble).mapWithIndex {
//      (value, index) => if (index >= nDemandOffers) -1.0 else 1.0
//    }
//    val balancing = LinearConstraintBuilder(balancingCoefficients)(fromDouble).equ(0.0)
//
//    // Upper bound on supplied energy per offer
//    val upperBounds = (demand("energies") ++ supply("energies")).zipWithIndex.map {
//      case (max, index) => LinearConstraintBuilder(e[ContinuousVariable](nOffers, index)(fromDouble))(fromDouble).le(max)
//    }
//
...
//      .max(LinearObjectiveFunction[ContinuousVariable](Constants(costVector: _*))(fromDouble))
...
//    val costVector = demand("prices") ++ supply("prices").map(_ * -1.0)
//    val objectiveFunction = LinearObjectiveFunction[ContinuousVariable](Constants(costVector: _*))(fromDouble)
//    val tableau = StandardSimplex.solve(DualTableau.max(objectiveFunction).subjectTo(constraints.toSeq: _*))
//
